...
decomp_total |>
  ...
  mutate(educ = factor(
    educ,
    levels = c("Educ. component", "Higher", "Secondary", "Primary")
  )) |>
  ggplot(aes(y = educ, x = margin, fill = educ)) +
  geom_col() +
  guides(color = "none") +
  theme_minimal() +
...
fig <- decomp_total |>
  ...
  summarise(
    valuersc = sum(result_rescaled),
    value = sum(result),
    .groups = "drop"
  ) |>
  mutate(cause = factor(cause), cause = reorder(cause, -valuersc)) |>
  ...
  theme(
    ...
    # Set angle to 0 and adjust hjust
