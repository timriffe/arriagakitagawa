...
\noindent where $\omega$ is the highest (open) age group, and $\Delta^s$ is the subgroup-specific ($s$) difference in life expectancy being decomposed, and it is composed of age-specific contributions, ${}_n\Delta_x^s$, which can be calculated following Arriaga's decomposition method following eq \eqref{eq_arriaga1}. I'll use a lifetable radix of 1, meaning $\ell_0 = 1$ to reduce the formula a bit. 
...
Equation \eqref{eq_arriaga1} is the first pass of our symmetrical decomposition, where we move _forward_ in time from $t=1$ to $t=2$, so please permit . It
