...
\noindent where the $S$ total subgroup fractions comprise a mixing distribution, $1 = \sum_s^S \pi^s$. 
...
The quantity ${}_n\Delta_x^s$ given in eq \eqref{eq_arriaga3} is on the scale of group $s$ specific life expectancy changes, but to know its net impact on overall life expectancy change we rescale per eq \eqref{eq_rescale} to match the effect sizes from eq \eqref{eq_kit_e0}
